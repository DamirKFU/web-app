...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = async (data: LoginFormData) => {
    try {
      const headers = await addCSRFToken();
      const response = await fetch(`${API_BASE_URL}${API_CONFIG.endpoints.login}`, {
        method: 'POST',
        headers: {
          ...headers,
  ...
  const register = async (data: RegisterFormData) => {
    try {
      const headers = await addCSRFToken();
      const response = await fetch(`${API_BASE_URL}${API_CONFIG.endpoints.register}`, {
        method: 'POST',
        headers,
        credentials: 'include',
        body: JSON.stringify(data),
      ...
      if (response.ok) {
        setIsAuthenticated(false); // регистрация ≠ автоматический вход

        toast({
          title: "Успешно",
          description: "Регистрация прошла успешно",
        });
        navigate("/register/success"); // ← вот перенаправление
